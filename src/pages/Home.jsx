...
import { offersApi, feedbackApi } from '../lib/api';
...
const Home = () => {
  ...
  // Flip DISABLE_SCRUB_ON_TOUCH to true if QA reveals stuttering on iOS/Android
  const DISABLE_SCRUB_ON_TOUCH = false;
  const [isTouchFallback] = useState(
    () => DISABLE_SCRUB_ON_TOUCH && typeof window !== 'undefined' && 'ontouchstart' in window
  );
  ...
  // Spring-smoothed progress — eases video.currentTime instead of snapping on every scroll tick
  ...
  // showPoster is derived before the event-listener hook so the listener can read it
  const showPoster = prefersReducedMotion || videoError || isTouchFallback;

  // First visible frame — skip the black intro at t=0
  const SCRUB_START = 0.3;

  // Scrub: drive video.currentTime from the spring value — never call this during render
  useMotionValueEvent(smoothProgress, 'change', (latest) => {
    if (showPoster) return;
    const video = videoRef.current;
    if (!video || video.readyState < 2 || !video.duration) return;
    // Map [0,1] scroll to [SCRUB_START, duration] so we never land on a black frame
  ...
  // Switch hero text panel based on scroll thirds
  ...
  const videoOpacity = useTransform(scrollYProgress, [0.75, 1], [1, 1]);
    ...
    <>
      <PageTransition>
        ...
        <div ref={heroRef} className={`-mt-[72px] ${showPoster ? 'h-[100dvh]' : 'h-[300vh]'}`}>
          <section className="sticky top-0 h-[100dvh] min-h-[480px] w-full overflow-hidden flex items-stretch bg-jet">
            ...
            <div className="absolute left-6 top-1/2 -translate-y-1/2 z-30 hidden md:flex flex-col items-center" style={{ direction: 'ltr' }}>
              <span className="font-sans text-[11px] text-white font-semibold tracking-[0.2em] mb-3">01</span>
              <div className="w-[2px] h-6 bg-gold rounded-full mb-3" />
              <div className="flex flex-col gap-[4px]">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="w-px h-[3px] bg-white/20" />
                ))}
              </div>
              <span className="font-sans text-[11px] text-white/30 tracking-[0.2em] mt-3">03</span>
            </div>

            {/* ── VIDEO PANEL (first in DOM → RTL flex places it on the RIGHT) ── */}
            {/* items-start + pt-[88px] keeps portrait card below the 72px navbar — no dark bleed */}
            <div className="absolute inset-0 z-0 md:relative md:inset-auto md:flex-1 md:flex md:items-start md:justify-center md:pt-[88px] md:pb-6 md:pe-14 md:ps-2">
              ...
              {/* Portrait frame — full-screen on mobile, 9:16 card on desktop */}
              <div className="
                w-full h-full
                md:h-full md:w-auto md:aspect-[9/16]
                md:max-w-[340px] lg:max-w-[370px] xl:max-w-[400px]
                md:rounded-2xl overflow-hidden relative
                shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_20px_80px_rgba(0,0,0,0.75),0_0_120px_rgba(212,175,55,0.06)]
              ">
                ...
                {/* Subtle vignette overlay — depth without hiding content */}
            ...
            {/* ── TEXT PANEL (second in DOM → RTL flex places it on the LEFT) ── */}
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center pt-16 pb-8
                            md:relative md:inset-auto md:w-[44%] md:flex-none md:items-start md:justify-center md:text-start md:pl-14 lg:pl-20 xl:pl-28 md:pr-10 md:pt-0 md:pb-0">

              {/* Mobile gradient — strong bottom so CTAs remain readable */}
              ...
              <div className="relative z-10 px-6 md:px-0 w-full">
                ...
                {/* Ghost large panel number — decorative background element */}
                <div
                    ...
                    return (
                      ...
                      >
                        ...
                        <motion.div
                          ...
                          className="inline-flex items-center gap-2.5 mb-8 bg-white/[0.06] border border-white/[0.12] backdrop-blur-sm px-4 py-2 rounded-full"
                        >
                        ...
                        {/* Headline — word 0 whisper small, rest large */}
                        <div className="mb-7 w-full">
                          <motion.span
                            className="block font-serif text-2xl md:text-3xl text-white/20 font-light leading-none mb-3 drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)]"
                            initial={{ opacity: 0, x: 20 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ delay: 0.18, duration: 0.55, ease: [0.25, 1, 0.5, 1] }}
                          >
                            {panel.words[0]}
                          </motion.span>
                          ...
                          <div className="flex flex-wrap justify-center md:justify-start items-baseline gap-x-3">
                            {panel.words.slice(1).map((word, i) => (
                              <motion.span
                                ...
                                transition={{ delay: 0.28 + i * 0.14, duration: 0.75, ease: [0.25, 1, 0.5, 1] }}
                              >
                                {word}
                              </motion.span>
                            ))}
                          ...
                          <motion.div
                            ...
                            transition={{ delay: 0.7, duration: 0.9, ease: [0.25, 1, 0.5, 1] }}
                          />
                        </div>
                        ...
                        <motion.div
                          ...
                          className="hidden md:flex items-stretch divide-x divide-x-reverse divide-white/10 border border-white/10 mb-8 overflow-hidden"
                        >
                          {[
                        ...
                        <motion.p
                          ...
                          className="font-sans text-xs sm:text-sm md:text-xs lg:text-sm text-white/45 max-w-[270px] mb-9 leading-[1.95] drop-shadow-[0_1px_5px_rgba(0,0,0,0.6)]"
                        >
                          {panel.subtitle}
                        </motion.p>
            ...
            >
              <span className="font-sans text-[9px] text-white/35 tracking-[0.35em] uppercase mb-1">اسحب للأسفل</span>
        ...
        <section className="py-24 md:py-36 bg-jet text-white relative overflow-hidden border-y border-gold/10">
          {/* Subtle glowing gold orb in the background */}
        ...
        {showTestimonialsSection && testimonials.length > 0 && (
          <section className="relative bg-white py-24 md:py-36 border-t border-gold/10 overflow-hidden">
            ...
            {/* Custom Local Style Block for Performance-Optimized CSS Keyframes */}
            <style>{`
              @keyframes waveSlow {
              ...
              .animate-wave-slow {
                animation: waveSlow 25s ease-in-out infinite;
                transform-origin: center;
              }
              .animate-wave-fast {
                animation: waveFast 18s ease-in-out infinite;
                transform-origin: center;
              }
              .animate-shimmer-vertical {
                animation: shimmerVertical 7s cubic-bezier(0.4, 0, 0.2, 1) infinite;
              }
            `}</style>

            {/* Vertical Golden Shimmer Threads */}
            ...
            {/* Swaying Golden Aroma Wave Lines */}
              ...
              >
                {testimonials.map((t) => (
                  ...
                  >
                    ...
                    {/* Customer details with isolation bdi */}
